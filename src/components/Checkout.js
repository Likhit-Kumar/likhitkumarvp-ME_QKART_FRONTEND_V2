...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    if(getTotalCartValue(items) > balance) {
      ...
      return false
    } else if(addresses.selected === "" && addresses.all.length > 0) {
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
              {
                ...
                addresses.all.map(address => (                 
                  <Box onClick={() => setAddresses({...addresses, selected: address._id})} className={addresses.selected === address._id ? "address-item selected" : "address-item not-selected"} key={address._id}>
                    <Box ml={1}>
                      <p type="text">{address.address}</p>
                    </Box>
                    <Box><Button variant="text" onClick={() => { deleteAddress(token, address._id)}}><Delete sx={{ color: "#00A278" }} ml={2} /> Delete</Button></Box>
                  </Box>
                )) : (
                  <Typography my="1rem">
